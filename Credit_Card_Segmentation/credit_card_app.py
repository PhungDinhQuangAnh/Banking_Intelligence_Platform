...
def run_credit_card_app():
    ...
    st.markdown("""
        ...
        /* Hệ thống thẻ số liệu KPI Grid - Nổi bật trên nền trắng nhờ viền đậm và shadow sâu */
        .kpi-grid {
            display: grid;
        ...
        .kpi-card {
            ...
            border-radius: 16px;
            padding: 16px;
            box-shadow: 0 10px 20px -3px rgba(15, 23, 42, 0.08), 0 4px 6px -4px rgba(15, 23, 42, 0.08);
            transition: all 0.25s cubic-bezier(0.4, 0, 0.2, 1);
        }
        .kpi-card:hover {
            transform: translateY(-6px);
            box-shadow: 0 25px 30px -5px rgba(15, 23, 42, 0.16), 0 12px 16px -6px rgba(15, 23, 42, 0.16);
        }
        .kpi-label {
            display: flex;
            align-items: center;
    ...
    with tab_dashboard:
        ...
        for group_id in STRATEGIES.keys():
            ...
            card_item = f'<div class="kpi-card" style="border: 2.5px solid {color}; background: linear-gradient(180deg, #FFFFFF 50%, {bg_light} 100%);"><div class="kpi-label" style="color: {color}; display: flex; align-items: center; gap: 6px;"><span class="material-icons" style="font-size: 18px; font-family: \'Material Icons\' !important;">{icon_name}</span><span>{short_label}</span></div><div class="kpi-num">{value_str}</div><div class="kpi-text" style="color: {color};">{group_name}</div></div>'
            kpi_html += card_item  
        kpi_html += '</div>'
        ...
        with c_left:
            st.markdown("""
            <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 14px; margin-top: 15px;">
                <div style="background: #E0F2FE; padding: 4px; border-radius: 10px; display: flex; align-items: center; justify-content: center;">
            ...
            # Định nghĩa mảng 9 cột thuộc tính đồng nhất cho cả 2 kiểu bản đồ 
            tsne_custom_data = [
                "Nhóm",                   # index 0
                "BALANCE",                # index 1
                "PURCHASES",              # index 2
                "ONEOFF_PURCHASES",       # index 3 
                "INSTALLMENTS_PURCHASES", # index 4 
                "PURCHASES_TRX",          # index 5
                "PURCHASES_FREQUENCY",    # index 6
                "CASH_ADVANCE",           # index 7
                "CREDIT_LIMIT",           # index 8
                "PAYMENTS"                # index 9
            ]

            # Chuỗi định dạng Tooltip cấu trúc chuẩn hóa 9 cột
            tsne_hovertemplate = (
                "<b>%{customdata[0]}</b><br>"
            ...
            if chart_type == "Bản đồ Không gian 3D":
                ...
                fig.update_traces(
                    ...
                    hovertemplate=tsne_hovertemplate
                )
                fig.update_layout(
        ...
        with c_right:
            ...
            mini_fig.update_traces(
                hovertemplate="<b>Chỉ báo tài chính:</b> %{customdata[0]}<br>" +
                            "<b>%{customdata[1]}:</b> %{y:,.2f} $<extra></extra>"
            )

            # Cấu hình riêng nhãn trục X bằng mảng ngắt dòng để không ảnh hưởng tới ô hover
            short_labels = [
                "Số dư nợ<br>hiện tại",
                "Tổng tiền<br>mua sắm",
                "Rút tiền mặt<br>từ thẻ",
                "Hạn mức<br>tối đa thẻ",
                "Số tiền<br>đã trả lại"
            ]
            ...
            mini_fig.update_layout(
                ...
                xaxis=dict(
                    ...
                    ticktext=short_labels, # Chỉ đè chữ ngắt dòng lên đúng bề mặt trục X
    ...
    with tab_infomation:
        ...
        <style>
            ...
            /* Lưới hiển thị 9 chỉ báo dữ liệu */
            .data-card-grid {
                display: grid;
            ...
            /* Khối hộp lớn có hiệu ứng Hover, Bóng đổ và Viền mịn */
            .premium-card {
                background: #FFFFFF;
                border: 1px solid #E2E8F0;
                border-radius: 14px;
            ...
            /* Thẻ con hiển thị tên cột bên trong lưới */
            .data-card {
                background: #F8FAFC;
                border: 1px solid #E2E8F0;
                border-radius: 10px;
       ...
       # Bước 1
        ...
        <div class="premium-card" style="margin-bottom: 15px;">
            <div style="background: #F8FAFC; padding: 20px; border-radius: 12px; border-left: 4px solid #60A5FA; box-shadow: 0 1px 3px 0 rgba(0, 0, 0, 0.05); margin-bottom: 20px;">
        ...
        # Bước 2
        st.markdown("""
        <div class="premium-card" style="margin-top: 25px; margin-bottom: 25px;">
